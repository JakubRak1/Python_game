...
class Weapon:
    ...
    # Constructor to create weapon object
        self.id = id
        self.name = name
        self.dmg = dmg
        self.hp = hp
    def __repr__(self) -> str:
    # Return str to display weapon object info
...
class Unit:
    ...
    def __repr__(self) -> str:
        return (f'<Unit id: {self.id}, name: {self.name}, hp: {self.hp}, dmg: {self.dmg}, exp: {self.exp}, needed exp to level up: {self.advance}>')
    
    
    def get_enemy_info (self) -> str:
        return (f'Enemy Name: {self.name.upper()}, HP:{self.hp}, DMG:{self.dmg}')
